...
\label{bem:8.18}
\begin{enumerate}[(a)]
\item $p(x) = \sum_{\abs\alpha} a_\alpha x^\alpha$ heißt \idx{Symbol}\index{Differentialoperator!Symbol} des Differentialoperators $p(D)$.
\item Sei $a \in \mathcal O_M$, dann gilt mit Satz~\ref{satz:8.14}, Theorem~\ref{theorem:8.12} und Theorem~\ref{theorem:8.7}, $a(D) := \F^{-1} a(\xi) \F$ definiert Abbildungen $\mathcal S(\R^n) \ra \mathcal S(\R^n),\mathcal S'(\R^n) \ra \mathcal S'(\R^n)$. $a(D)$ heißt \idx{Pseudodifferentialoperator} mit Symbol $a$.
\end{enumerate}
\end{bem}
...
\end{align*}
d.h. $p(\xi) = 1+\sum_{j=1}^n \xi_j^2 = 1+\abs\xi^2$ ist \idx{Symbol} von $1-\Delta$. Dann folgt (Übung!)
\begin{align*}
